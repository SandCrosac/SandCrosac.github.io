...
<div class="g-view container" style="height: auto;">
    ...
    <article class="thread_content">
        ...
        <script>
            $(function(){
                $("head").append('<script src="{#WWW}View\/nd_mobile\/js\/fly-zomm-img.min.js"><\/script>')
                $('#img_list').FlyZommImg();
            })
        </script>
        {/if}
    ...
    <div id="post_list">
        ...
        <div class="thread_post">
            ...
            <?php
                ...
                $sz=0;
                if(isset($match[1][0])){
                    foreach ($match[1] as  $key => $vvas) {

                        if(substr_count($vvas,'data:image/') || substr_count($vvas,';base64') || strpos($vvas,'/emoji/') !== FALSE || empty($vvas)){
                            continue;
                        }
                        $imgs[] =$vvas;
                        $content = str_replace($match[0][$key],"",$content);
                    }
                }
                $v['content'] = $content;
            ?>
            <!--{hook t_m_post_content_top}-->
                {$v.content}
                {if $imgs}
                <div class="img_list" id="img_list_{$v.pid}">
                    {foreach $imgs as $img}
                    <li>
                        <img src="{$img}" alt="" style="height: 110px;">
                    </li>
                    {/foreach}
                </div>
                {/if}
